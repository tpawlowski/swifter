...
public class HttpParser {
    ...
    private func extractQueryParams(_ url: String) -> [(String, String)] {
        ...
        let query = url[queryStart..<url.endIndex]
        return query.components(separatedBy: "&")
            .reduce([(String, String)]()) { (c, s) -> [(String, String)] in
                guard let nameEndIndex = s.firstIndex(of: "=") else {
                    return c
                }
                guard let name = s[s.startIndex..<nameEndIndex].removingPercentEncoding else {
                    return c
                }
                ...
                guard let value = s[valueStartIndex..<s.endIndex].removingPercentEncoding else {
                    return c + [(name, "")]
                }
                return c + [(name, value)]
        }
